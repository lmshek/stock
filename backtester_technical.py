...
from technical.rsi import rsi14, rsi_sell
...
class backtester(simulator):
    ...
    def __init__(self, stocks_list, model, model_version, capital, start_date, end_date, sell_perc = 0.08, hold_till = 10, stop_perc = 0.004, no_of_splits = 5):
        ...
        folder_name = f'{str(self.model.__name__)}_{self.model_version}_{self.hold_till}_{self.sell_perc}_{self.stop_perc}'
    ...
    def backtest(self):
        ...
        while self.day <= self.end_date:
            ...
            if not(self.day in hk_holidays or hk_holidays._is_weekend(self.day)):                
                ...
                for s in stocks:
                    recommended_action, current_price = stoch_sell(s, self.buy_orders[s][3], self.buy_orders[s][0], self.day, \
    ...
    def save_results(self):
        ...
        params = [self.hold_till, self.sell_perc, self.stop_perc, self.start_date, self.end_date]
